...
def compare(left: Path, right: Path) -> dict:
    ...
    return {"left": str(left), "right": str(right), "confusion_score": score,
            "level": "excellent" if score <= 20 else "acceptable" if score <= 30 else "runtime_fix" if score <= 40 else "adjust" if score <= 55 else "regenerate",
...
if __name__ == "__main__":
    ...
    result = {"deterministic": True, "asset_count": len(args.paths), "pairs": pairs}
